...
router.post("/",verifyUser,async(req,res)=>{
    ...
    const recipient = await prisma.recipient.create({
        data:{
            name,email,companyName,userId:userid
        }
    })
...
router.get("/",verifyUser,async(req,res)=>{
    try{
        ...
        return res.status(200).json({data:allRecipient,message:"Recipient fetched Successfully"})
    }
    catch(error){
         return res.status(500).json({ message: "Intenal server error" });
    }
...
router.put("/:id",verifyUser,async(req,res)=>{
    ...
    return res.status(200).json({data:updatedRecipient,message:"Updated successfully "})
    }
    catch(error){
    console.log(error);
    return res.status(500).json({ message: "Intenal server error" });
...
router.delete("/:id",verifyUser,async(req,res)=>{
    ...
    const deletedRecipient = await prisma.recipient.delete({where:{
        id
    }})
    return res.status(200).json({message:"Recipient deleted successfully",data:deletedRecipient})
    }
    catch(error){
        return res.status(500).json({ message: "Intenal server error" });
    }
